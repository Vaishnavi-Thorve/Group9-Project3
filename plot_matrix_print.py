        ...
        for num in range (self.num_iterations):
            ...
            for i, room in enumerate(self.rooms):
                if self.rank == i:
                    ...
                    sol = solve_linalg(room.boundary_mask, u, self.dx, i)          # might be an issue here
...
def build_linalg(boundary_mask, Nx, Ny, dx, room):
    ...
    for j in range(Nx):
        for k in range(Ny):
            ...
            b.append(rhs)
            if dx == 1/3:
                createLatexDocument(np.array(A), np.array(b), str(room))
...
def solve_linalg(boundary_mask, temperature_grid, dx, i):
    shape = temperature_grid.shape
    print(f'Shape of the temperature_grid is: {shape}')
    n, m = shape

    A, b = build_linalg(boundary_mask, n, m, dx, i)
