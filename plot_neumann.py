        ...
        for num in range (self.num_iterations):
            ...
            for i, room in enumerate(self.rooms):
                if self.rank == i:
                    ...
                    sol = solve_linalg(room, room.boundary_mask, u, self.dx)          # might be an issue here
        ...
        for direction, info in room.adjacent_rooms.items():
            ...
            if gamma_type == 'Neumann':
                if direction == 'right':
                    ...
                    room.boundary_mask[start+1:end+1, -1] = flux * dx
                    ...
                    room.boundary_mask[start+1:end+1, 0] = flux * dx
                                                            
        return temperature_grid 


def build_linalg(room, boundary_mask, Nx, Ny, dx):
    N = Nx * Ny
    A = []
    b = []
    
    direction, info = room.adjacent_rooms.items()            
    gamma_type = info['gamma_type']
    boundary_positions = info['boundary_positions']
    start = boundary_positions['start']
    end = boundary_positions['end']
    
    # go through all elements in grid inside the 
    for j in range(Nx):
        for k in range(Ny):
            row = np.zeros(N)
            rhs = 0
            ...
            # if the boundary is neumann and the element is in the height of that border
            if gamma_type == 'Neumann' and k >= start and k <= end:
                # check where the adjacent room lays and only update the diagonal element if the neighbouring element really is outside the mesh
                if direction == 'left' and j-1 < 0:
                    diag += 1
                elif direction == 'right' and j+1 >= Nx:
                    diag += 1
...
# find index of studied element 
def get_index(j, k, Ny):
    return j * Ny + k


def solve_linalg(room, boundary_mask, temperature_grid, dx):
    shape = temperature_grid.shape
    print(f'Shape of the temperature_grid is: {shape}')
    n, m = shape

    A, b = build_linalg(room, boundary_mask, n, m, dx)
